...
public class OBJ : MonoBehaviour
{
    ...
    public IEnumerator Load(string path)
    {
        ...
        yield return loader;
        //put code to separate strings under here
    ...
    public void DeleteEmptyObjects(GameObject[] mm)
    {
        foreach (GameObject go in mm)
        {
            // if (go.GetComponent<MeshFilter>().mesh.name.StartsWith("None"))
            // if (go.GetComponent<MeshFilter>().mesh.name.Equals(""))
               //if(go.GetComponent<MeshRenderer>().material.shader.name == null || go.GetComponent<MeshFilter>().mesh == null || go.GetComponent<MeshRenderer>().materials[0].name.StartsWith("None") || go.GetComponent<MeshFilter>().mesh.name.Equals("") || go.GetComponent<MeshRenderer>().materials.Length == 0)
                //{
    ...
    void SetRootPosition() { //sets root object position and rotation
        ...
        Vector3 buildingpos = transform.TransformPoint(transform.GetComponentInChildren<MeshFilter>().mesh.bounds.min.x, 55, transform.GetComponentInChildren<MeshFilter>().mesh.bounds.min.z);
